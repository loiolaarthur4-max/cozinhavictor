...
def scanner_js():
    ...
    <script>
        function onScanSuccess(decodedText) {
            // Encontra o input do Streamlit e preenche
...
with st.sidebar:
    ...
    nome_f = st.text_input("Nome do Produto (ou escaneie o código)")
...
if busca:
    ...
    with abas[-1]:
        for r in cursor.execute("SELECT nome FROM historico_produtos").fetchall(): st.write(f"- {r[0]}")import streamlit as st
import streamlit.components.v1 as components
from datetime import datetime, date
import sqlite3
...
if busca:
    ...
    for i, loc in enumerate(locais):
        with abas[i]:
            render_lista([p for p in produtos if p['local'] == loc])
    with abas[-1]:
        for r in cursor.execute("SELECT nome FROM historico_produtos").fetchall(): st.write(f"- {r[0]}")

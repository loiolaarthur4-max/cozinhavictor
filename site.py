...
# AQUI ESTÃO AS COLUNAS (DEVE VIR ANTES DE QUALQUER 'with col1')
...
with col1:
    ...
    sel_nome = st.selectbox("Histórico", opcoes_nome, key="sel_n")
    nome_f = st.text_input("Ou novo nome:", value=d.get("nome", ""))
    nome_final = nome_f if nome_f else sel_nome
    
    # Marcas
    st.subheader("Marca")
    opcoes_marca = [""] + get_historico("marca")
    sel_marca = st.selectbox("Histórico", opcoes_marca, key="sel_m")
    marca_f = st.text_input("Ou nova marca:", value=d.get("marca", ""))
    ...
    for i, local in enumerate(locais):
        with abas[i]:
            for item in [p for p in produtos if p['local'] == local]:
                ...
                st.markdown(f'<div style="padding: 12px; background-color: {bg}; color: #ffffff; margin-bottom: 10px; border-radius: 8px;"><b>{item["nome"]}</b> ({item["marca"]})<br>📦 {item["quantidade"]:.0f}x {item["peso"]}{item["unidade"]} | 📅 {item["validade"].strftime("%d/%m/%Y")} ({dias} dias)</div>', unsafe_allow_html=True)
